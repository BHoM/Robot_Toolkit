...
namespace BH.Adapter.Robot
{
    public partial class RobotAdapter
    {
        ...
        {
            List<BarDeformation> barDeformations = new List<BarDeformation>();
            ...
            while (ret != IRobotResultQueryReturnType.I_RQRT_DONE)
            {
                ...
                while (isOk)
                {
                    ...
                    double ux = row.IsAvailable((int)IRobotExtremeValueType.I_EVT_DISPLACEMENT_BAR_UX) ? row.GetValue((int)IRobotExtremeValueType.I_EVT_DISPLACEMENT_BAR_UX) : 0;
                    double uy = row.IsAvailable((int)IRobotExtremeValueType.I_EVT_DISPLACEMENT_BAR_UY) ? row.GetValue((int)IRobotExtremeValueType.I_EVT_DISPLACEMENT_BAR_UY) : 0;
                    double uz = row.IsAvailable((int)IRobotExtremeValueType.I_EVT_DISPLACEMENT_BAR_UZ) ? row.GetValue((int)IRobotExtremeValueType.I_EVT_DISPLACEMENT_BAR_UZ) : 0;

                    double rx = row.IsAvailable((int)IRobotExtremeValueType.I_EVT_DISPLACEMENT_BAR_RX) ? row.GetValue((int)IRobotExtremeValueType.I_EVT_DISPLACEMENT_BAR_RX) : 0;
                    double ry = row.IsAvailable((int)IRobotExtremeValueType.I_EVT_DISPLACEMENT_BAR_RY) ? row.GetValue((int)IRobotExtremeValueType.I_EVT_DISPLACEMENT_BAR_RY) : 0;
                    double rz = row.IsAvailable((int)IRobotExtremeValueType.I_EVT_DISPLACEMENT_BAR_RZ) ? row.GetValue((int)IRobotExtremeValueType.I_EVT_DISPLACEMENT_BAR_RZ) : 0;


                    double position = (1 / (System.Convert.ToDouble(division) - 1)) * (System.Convert.ToDouble(idPoint) - 1);

                    BarDeformation barDeformation = new BarDeformation
                    {
                    ...
                    barDeformations.Add(barDeformation);
                    isOk = rowSet.MoveNext();

                }
            }
            return barDeformations;
        }
